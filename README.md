...
입력이 남이 준 영상이나 폴더일 수 있다는 전제로 다음을 지킵니다.

- **`shots.json`을 믿지 않습니다** — `--extract-only`가 읽는 캐시의 에피소드 ID는 영문·숫자·한글·`_`·`-`만 허용하고, 영상은 원본 폴더 안의 파일명만 받으며, 출력 폴더가 저장 위치 밖으로 풀리면 그 항목을 건너뜁니다. 공유 폴더에 누가 조작한 캐시를 놓아도 다른 폴더의 파일을 지우거나 읽을 수 없습니다
- **자기가 만든 파일만 지웁니다** — 재실행 시 출력 폴더에서 `<태그>_cut###_…jpg` 패턴에 맞는 것만 정리하고, 사용자가 둔 다른 파일은 건드리지 않습니다
- **ffmpeg는 절대경로로만 부릅니다** — Windows는 PATH보다 현재 폴더를 먼저 뒤지기 때문에, 이름만으로 부르면 exe 옆에 놓인 가짜 `ffmpeg.exe`가 실행될 수 있습니다. 번들 `bin/` → PATH 순으로 직접 찾되 현재 폴더는 검색에서 뺍니다
- **영상 파서는 ffmpeg입니다** — 악성 영상은 ffmpeg 자체의 취약점을 노립니다. 출처를 모르는 영상은 최신 ffmpeg로 돌리세요. 릴리스 exe에 동봉된 ffmpeg 버전은 릴리스 노트에 적혀 있습니다
